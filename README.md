...
`git remote set-url origin` - установи адрес удалённого репозитория (origin), куда будут загружаться и
откуда будут выгружаться новые коммиты.
  
`git fetch` - собери коммиты, которых нет в текущей ветке, из 
целевой, а затем сохрани их в локальном репозитории, 
не изменяя его состояние.  

### Откладывание изменений
`git stash` - спрячь изменения, которые ещё не готовы стать коммитом, в хранилище.  
`git stash list` - покажи список отложенных изменений.  
`git stash pop` - достать последний набор отложенных изменений, 
удали его из хранилища и примени к исходному коду.  
`git stash apply stash@{n}` - верни конкретные изменения по их 
номеру из списка (n).  

### Объединение и разрешение конфликтов
`git pull --merge` - объедини коммиты и проанализируй локальные 
